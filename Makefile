...
run: phts
	PHTS_ADMIN_EMAIL=test@test.local\
			...
			PHTS_DB_HOST=localhost\
			PHTS_DB_PASSWORD=$(DEV_DB_PASSWORD)\
			PHTS_DB_SSLMODE=false\
			PHTS_DB_USER=$(DEV_DB_USER)\
			PHTS_INITIAL_USER=test@test.local\
			PHTS_INITIAL_USER_PASSWORD=asdf\
			PHTS_MINIO_ACCESS_KEY=$(MINIO_ACCESS_KEY)\
			PHTS_MINIO_BUCKET=$(MINIO_BUCKET)\
			PHTS_MINIO_ENDPOINT=localhost:9000\
