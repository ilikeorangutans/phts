...
export class ShareService {
  ...
  listForPhoto(collection: Collection, photo: Photo): Observable<Array<Share>> {
    ...
    return this.http.get<Array<Share>>(url).pipe(
      map((response) => {
  ...
  sharesForPhoto(collection: Collection, photo: Photo): PhotoShares {
    return new PhotoShares(this, collection, photo);
  }
}

export class PhotoShares {
  private readonly _all: BehaviorSubject<Array<Share>> = new BehaviorSubject<
    Array<Share>
  >([]);

  readonly all: Observable<Array<Share>> = this._all.asObservable();
  ...
  add(request: ShareRequest): Observable<Share> {
    const x = this.shareService.save(this.collection, this.photo, request);

    x.pipe(first()).subscribe((_) => this.reload());
